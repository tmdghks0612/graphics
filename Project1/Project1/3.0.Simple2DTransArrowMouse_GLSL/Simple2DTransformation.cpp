...
#define ACC 0.001
#define PAWNSPEED 0.2f
#define UPDATERATE 1

GLfloat carspeed[3] = { 0.2f , -0.2f, 0.2f};
GLfloat carcoord[3] = { -300.0f, 400.0f, -200.0f };
GLfloat carheight[3] = { 250.0f, 0.0f, -250.0f };
int win_width = 0, win_height = 0; 
bool startflag = false;
float centerx = 0.0f, centery = 0.0f, rotate_angle = 0.0f;
float boundary = 15.0f;
float cake_angle = 0.0f;

float hat_height = 0.0f;
float hat_speed = 0.5f;
float cocktailx = 100.0f;
float cocktaily = 0.0f;
float cocktail_speedx = 0.3f;
float cocktail_speedy = 0.3f;

float alien_scale = 0.0f;
float alien_rotation = 0.0f;
int alien_flag = 0;

GLfloat obstacle[8][2] = {
	{-500.0f,100.0f},
	{-200.0f,-400.0f},
	{0.0f,150.0f},
	{150.0f,0.0f},
...
#define ALIEN_EYE 3
#define ALIEN_MOUTH 4
GLfloat alien_head[6][2] = { {0.0,0.0}, {-5.0, -1.0}, { -4.0, -4.0 },{ -0.0, -4.5 } ,{4.0,-4.0}, {5.0,-1.0}};
GLfloat alien_lefthat[5][2] = { { -5.0, -1.0 },{ -9.0, 4.0 },{ -3.0, 0.0 },{0.0,4.0},{ 0.0, 0.0 } };
GLfloat alien_righthat[5][2] = { {0.0,0.0}, {0.0,4.0}, {3.0,0.0},{ 9.0, 4.0 },{5.0,-1.0} };//,{3.0,0.0} ,{0.0,4.0}, {0.0,0.0}};
GLfloat alien_lefteye[3][2] = { {-1.0,-1.0},{-4.0,-2.0},{-3.5,-3.5} };
GLfloat alien_righteye[3][2] = { {1.0,-1.0},{4.0,-2.0},{3.5,-3.5} };
GLfloat alien_mouth[4][2] = { {0.0,-4.5},{4.0,-4.0},{0.0,-6.0},{-4.0,-4.0} };
GLfloat alien_color[9][3] = {
	{ 183 / 255.0f, 110 / 255.0f, 121 / 255.0f },
	{ 255 / 255.0f, 255 / 255.0f, 255 / 255.0f },
	{ 0 / 255.0f, 0 / 255.0f, 0 / 255.0f },
	{ 10 / 255.0f, 110 / 255.0f, 180 / 255.0f },
...
void prepare_alien() {
	GLsizeiptr buffer_size = sizeof(alien_head) + sizeof(alien_lefthat) + sizeof(alien_righthat)+sizeof(alien_lefteye)+sizeof(alien_righteye)+sizeof(alien_mouth);
	...
	glBufferSubData(GL_ARRAY_BUFFER, sizeof(alien_head)+ sizeof(alien_lefthat), sizeof(alien_righthat), alien_righthat);
	glBufferSubData(GL_ARRAY_BUFFER, sizeof(alien_head) + sizeof(alien_lefthat) + sizeof(alien_righthat), sizeof(alien_lefteye), alien_lefteye);
	glBufferSubData(GL_ARRAY_BUFFER, sizeof(alien_head) + sizeof(alien_lefthat) + sizeof(alien_righthat)+sizeof(alien_lefteye),sizeof(alien_righteye),alien_righteye);
	glBufferSubData(GL_ARRAY_BUFFER, sizeof(alien_head) + sizeof(alien_lefthat) + sizeof(alien_righthat) + sizeof(alien_lefteye)+ sizeof(alien_righteye),sizeof(alien_mouth),alien_mouth);
...
void draw_alien() {
	...
	glUniform3fv(loc_primitive_color, 1, alien_color[ALIEN_EYE]);
	glDrawArrays(GL_TRIANGLE_FAN, 16, 6);

	glUniform3fv(loc_primitive_color, 1, alien_color[ALIEN_MOUTH]);
...
void tick(int)
{
	...
	if (startflag)
	{
		...
		cocktailx += cocktail_speedx;
		cocktaily += cocktail_speedy;

		if (centerx > (win_width / 2.0)*0.975 || centery > (win_height / 2.0)*0.95 || centerx < (-1 * (win_width / 2.0))*0.975 || centery < (-1 * (win_height / 2.0))*0.95)
		...
		if (cocktailx > 100)
		{
			cocktail_speedx = -0.3f;
		}
		else if (cocktailx < -100)
		{
			cocktail_speedx = 0.3f;
		}

		if (alien_scale < 3.0f && alien_flag == 0)
		{
			alien_rotation += 3.0;
			alien_scale += ACC;
		}
		else if (alien_rotation > 360.0f || alien_rotation == 0.0f)
		{
			alien_flag = 1;
			if (alien_scale > 0.0f)
			{
		...
		for (i=0; i < 8; ++i)
		{
			if (((centerx < obstacle[i][0] + boundary) && (centerx > obstacle[i][0] - boundary)) && ((centery < obstacle[i][1] + boundary) && (centery > obstacle[i][1] - boundary)))
			{
				glutLeaveMainLoop();
			}
		}
		for (i=0; i < 3; ++i)
		{
			...
			carcoord[i] += carspeed[i];
			if (((centerx < carcoord[i] + boundary) && (centerx > carcoord[i] - boundary)) && ((centery < carheight[i] + boundary) && (centery > carheight[i] - boundary)))
			{
				glutLeaveMainLoop();
			}
		}
...
/*void getmouse(int input)
{
	if (leftbuttonpressed && input==0)
	{
		//start game
		startflag = true;
		//glutTimerFunc(UPDATERATE, tick, 0);
	...
	else
	{
		while (!input)
		{
			glutTimerFunc(UPDATERATE, getmouse, 1);
		}
	}
...
void display(void) {
	...
	ModelMatrix = glm::rotate(ModelMatrix, cake_angle, glm::vec3(0.0f, 0.0f, 1.0f));
	...
	ModelMatrix = glm::translate(glm::mat4(1.0f), glm::vec3(win_width/4.0f+cocktailx, win_height/4.0f+cocktaily, 0.0f));
	ModelMatrix = glm::rotate(ModelMatrix, -30.0f*TO_RADIAN, glm::vec3(0.0f, 0.0f, 1.0f));
	...
	ModelMatrix = glm::rotate(ModelMatrix, 120.0f*TO_RADIAN, glm::vec3(0.0f, 0.0f, 1.0f));
	...
	for (i = 0; i < 8; ++i)
	{
		ModelMatrix = glm::translate(glm::mat4(1.0f), glm::vec3(obstacle[i][0], obstacle[i][1], 0.0f));
		ModelViewProjectionMatrix = ViewProjectionMatrix * ModelMatrix;
		glUniformMatrix4fv(loc_ModelViewProjectionMatrix, 1, GL_FALSE, &ModelViewProjectionMatrix[0][0]);
		draw_house();
	}
	...
	ModelMatrix = glm::rotate(ModelMatrix, alien_rotation*TO_RADIAN,glm::vec3(0.0f, 0.0f,1.0f));
	ModelMatrix = glm::scale(ModelMatrix, glm::vec3(alien_scale, alien_scale, 0.0f));
	ModelViewProjectionMatrix = ViewProjectionMatrix * ModelMatrix;
	glUniformMatrix4fv(loc_ModelViewProjectionMatrix, 1, GL_FALSE, &ModelViewProjectionMatrix[0][0]);
	draw_alien();
	//plane1
	...
	for (i = 0; i < 3; ++i)
	{
		ModelMatrix = glm::translate(glm::mat4(1.0f), glm::vec3(carcoord[i], carheight[i], 0.0f));
		ModelViewProjectionMatrix = ViewProjectionMatrix * ModelMatrix;
		glUniformMatrix4fv(loc_ModelViewProjectionMatrix, 1, GL_FALSE, &ModelViewProjectionMatrix[0][0]);
		draw_car();
	}
...
void register_callbacks(void) {
	...
	glutTimerFunc(UPDATERATE, tick, 0);
	//glutTimerFunc(UPDATERATE, getmouse, 0);
}
